...
;(function(){
  ...
  var Tag = (function() {
    ...
    */
    ...
    Tag.prototype.toListItem = function() {
      ...
      if (!this.html4 && this.html5)
      {
        newTag = "<p class='ui-li-count'>html5</p>";
        iconFile = 'html5.png';
      }

      return "<li data-tag-name='" + this.name + "' class='" + obsoleteClass + "'><a href='http://developer.mozilla.org/en/HTML/Element/" + this.name + "'><img class='ui-li-icon' src='images/" + iconFile + "'><h1>" + this + "</h1><p> " + this.explain + "</p>" + obsolete + newTag +"</a></li>";
    }
  ...
  // Make sure we have sort the tags
...
;(function($){
  ...
  app.view.renderAllTags = function() {
    $('#tags').empty();
    for (var i=0, len=app.tags.length; i<len; i++) {
      tag = app.tags[i];
  ...
  app.view.renderNewTags = function() {
    $('#tags').empty();
    for (var i=0, len=app.tags.length; i<len; i++) {
      tag = app.tags[i];
  ...
  app.view.renderObsoleteTags = function() {
    $('#tags').empty();
    for (var i=0, len=app.tags.length; i<len; i++) {
      tag = app.tags[i];
  ...
  app.view.renderTagsByType = function() {
    $('#tags').empty();
  ...
  function refreshList() {
    ...
    $("#tags").listview({
      ...
      autodividersSelector: function (li) {
        return li.data("tagName")[0].toUpperCase();
      }
    }).listview("refresh");
  }
...
;(function($){
  ...
  // add back btn for the nested list
  $.mobile.page.prototype.options.addBackBtn = true;

  // set the header color theme to black
  $.mobile.listview.prototype.options.headerTheme = "a";

  // Entry point
  $(function(){

    app.view.renderAllTags();
    ...
    $('#sort-type').click(function(){
      app.view.renderTagsByType();
    })

    $('#sort-new').click(function(){
      app.view.renderNewTags();
    })

    $('#sort-obsolete').click(function(){
      app.view.renderObsoleteTags();
    })
  });
