...
(function() {
  var Feedbacks = (function(){
    function Feedbacks(){
      this.url = 'https://api.mongolab.com/api/1/databases/great_limited_db/collections/feedbacks?apiKey=502d0b59e4b07320d21ab85e'
    }
...
(function() {
  var FeedbacksView = (function(){
    ...
    // listing the data    
    FeedbacksView.prototype.list = function() {
      $(this.element).empty();      
      for (var i=0, len=this.data.length; i<len; i++)
      {
        var feedback = this.data[i];
        var listItem = "<li><h2>" + feedback.name + "</h2><p>" + feedback.email + "</p><p>" + feedback.message + "</p>";
...
(function() {
  var Contact = (function(){
    function Contact(){
      this.url = 'https://api.mongolab.com/api/1/databases/great_limited_db/collections/contacts?apiKey=502d0b59e4b07320d21ab85e'
    }
    Contact.prototype.post = function (windows, mac, linux, gender, phone, message, callback) {
      $.ajax({
        url: this.url,
        type: "POST",
        contentType: "application/json",
  ...
  // export the Contact to global scope
  if (!this.greatLtd) this.greatLtd = {}
  this.greatLtd.Contact = Contact;
...
(function(){
  ...
  // export the ContactView to global scope
  if (!this.greatLtd) this.greatLtd = {}
  this.greatLtd.ContactView = ContactView;
...
(function(){  
  ...
  var TwitterQuery    = greatLtd.TwitterQuery,
      ...
      Feedbacks       = greatLtd.Feedbacks;
      FeedbacksView   = greatLtd.FeedbacksView;
      ContactView     = greatLtd.ContactView;
  ...
  $(function(){
      ...
      switch ($(event.target).attr('id')) {
        ...
        case 'list-feedback':          
          (new Feedbacks()).fetch(function(data){
            ...
            (new FeedbacksView(data, '#feedbacks-list')).list();
          })
      }
    ...
    (new ContactView()).handlePostButton();
